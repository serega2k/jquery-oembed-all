...
(function ($) {
            ...
            if (resourceURL !== null && resourceURL !== undefined) {
                ...
                for (var j = 0, l = shortURLList.length; j < l; j++) {
                    ...
                    if (resourceURL.match(regExp) !== null) {
                        ...
                        var ajaxopts = $.extend({
                            url: options.longUrlServiceUrl + encodeURIComponent(resourceURL),
                            dataType: 'json',
                            success: function (data) {
                                ...
                                if (!!!longUrl) {
                                    provider = $.fn.oembed.getOEmbedProviderByName('opengraph');
                                ...
                                if (provider !== null) {
                                    ...
                                    provider.maxWidth = settings.maxWidth;
                                    provider.maxHeight = settings.maxHeight;
                                    embedCode(container, longUrl, provider);
                                } else {
                                    settings.onProviderNotFound.call(container, longUrl);
                                }
                            },
                            error: function (ajax) {
                                var error = "";
                                ...
                                if (ajax.status < HTTP_REDIRECT_SECTION || ajax.status >= HTTP_CLIENT_ERROR_SECTION) {
                                    settings.onError.call(container, resourceURL, error);
                                }
                            }
    ...
    var settings;
    var HTTP_REDIRECT_SECTION = 300;
    var HTTP_CLIENT_ERROR_SECTION = 400;
    // Plugin defaults
    $.fn.oembed.defaults = {
        fallback: true,
        maxWidth: null,
        maxHeight: null,
        longUrlServiceUrl: null,
        bypassUrl: null,
        headParserUrl: null,
        includeHandle: true,
        embedMethod: 'auto',
    ...
    function success(oembedData, externalUrl, container) {
        oembedData.isPlainLink = oembedData.isPlainLink || false;
    ...
    function embedCode(container, externalUrl, embedProvider) {
        if ($('#jqoembeddata').data(externalUrl) != undefined && embedProvider.embedtag.tag != 'iframe') {
            var oembedData = $('#jqoembeddata').data(externalUrl);
            success(oembedData, externalUrl, container);
        } else if (embedProvider.yql) {
            // sometimes schooltube.com returns an URL with " character in it that does not matter but breaks the query,
            // URL-encoded links are not supported by query.yahooapis.com/v1/public/yql
            externalUrl = externalUrl.replace('"', '%22');
            var ajaxopts = $.extend({
                url: settings.headParserUrl,
                dataType: 'json',
                data: {
                    url: externalUrl,
                ...
                success: function (data) {
                    ...
                    var meta = { "url": externalUrl };

                    if (data.query == null) {
                        data.query = {};
                    }
                    if (data.query.hasError || data.query.results == null) {
                        ...
                        for (var i = 0, l = data.query.results.link.length; i < l; i++) {
                            if (data.query.results.link[i].hasOwnProperty("rel")) {
                                if (data.query.results.link[i].rel == "apple-touch-icon") {
                                    ...
                                    {
                                        var match = externalUrl.match(/^(([a-z]+:)?(\/\/)?[^\/]+\/).*$/);
                                        if (match && match.length)
                                        {
                                            meta["og:image"] = match[1] + data.query.results.link[i].href;
                                        }
                                        else
                                        {
                                            meta["og:image"] = "javascript:void(0);";
                                        }
                                    } else {
                    ...
                    oembedData.code = result.code || result;
                    oembedData.previewImage = meta['og:image'];
                    success(oembedData, externalUrl, container);
                },
                error: function (xhr, status) {
                    if (status < HTTP_REDIRECT_SECTION || status >= HTTP_CLIENT_ERROR_SECTION) {
                        settings.onError.call(container, externalUrl, embedProvider);
                    }
                }
            ...
            if (embedProvider.embedtag.tag !== '') {
                ...
                var width = settings.maxWidth || embedProvider.embedtag.width || 'auto';
                var height = settings.maxHeight || embedProvider.embedtag.height || 'auto';
                var src = externalUrl.replace(embedProvider.templateRegex, embedProvider.apiendpoint);

                src += embedProvider.embedtag.params || '';
            ...
            //some services can return HTTP302 or HTTP403 that cause some problems, so a proxy will be used to wrap such calls
            if (embedProvider.safeget == 'true' && settings.bypassUrl !== null && settings.bypassUrl !== '') {
                requestUrl = settings.bypassUrl + encodeURIComponent(requestUrl);
            }
            ajaxopts = $.extend({
                url: requestUrl,
                dataType: embedProvider.dataType || 'json',
                success: function (data) {
                    var oembedData = $.extend({}, data);
                    oembedData.previewImage = '';
                    switch (oembedData.type) {
                        ...
                        case "photo":
                            var embedData = $.fn.oembed.getPhotoCode(externalUrl, oembedData)
                            oembedData.code = embedData.code;
                            oembedData.previewImage = embedData.previewImage;
                            break;
                        case "video":
                        case "rich":
                ...
                error: function (xhr, status) {
                    if (xhr.status < HTTP_REDIRECT_SECTION || xhr.status >= HTTP_CLIENT_ERROR_SECTION) {
                        //Trying to render OGP in the case we got an error related to a format of the URL
                        if (xhr.status == 400) {
                            provider = $.fn.oembed.getOEmbedProviderByName('opengraph');
                            provider.params = getNormalizedParams(settings[provider.name]) || {};
                            provider.maxWidth = settings.maxWidth;
                            provider.maxHeight = settings.maxHeight;
                            embedCode(container, externalUrl, provider);
                        }
                        else {
                            settings.onError.call(container, externalUrl, embedProvider);
                        }
    ...
    $.fn.oembed.getPhotoCode = function (url, oembedData) {
        var data = { code: '', previewImage: '' };
        var alt = oembedData.title ? oembedData.title : '';
        alt += oembedData.author_name ? ' - ' + oembedData.author_name : '';
        alt += oembedData.provider_name ? ' - ' + oembedData.provider_name : '';

        if (oembedData.html) {
            data.code = "<div>" + oembedData.html + "</div>";
        } else if (oembedData.url) {
            data.code = '<div><a href="' + url + '" target=\'_blank\'><img src="' + oembedData.url + '" alt="' + alt + '"/></a></div>';
            data.previewImage = oembedData.url;
        } else if (oembedData.thumbnail_url) {
            var newURL = oembedData.thumbnail_url.replace('_s', '_b');
            data.code = '<div><a href="' + url + '" target=\'_blank\'><img src="' + newURL + '" alt="' + alt + '"/></a></div>';
            data.previewImage = newURL;
        } else {
    ...
    $.fn.oembed.getOEmbedProviderByName = function (name) {
        for (var i = 0; i < $.fn.oembed.providers.length; i++) {
    ...
    $.fn.oembed.OEmbedProvider = function (name, type, urlschemesarray, apiendpoint, extraSettings) {
        ...
        this.safeget = this.safeget || 'false';
    ...
    $.fn.oembed.providers = [
        ...
        new $.fn.oembed.OEmbedProvider("youtube", "video", ["youtube\\.com/watch.+v=[\\w-]+&?", "youtu\\.be/[\\w-]+", "youtube.com/embed"], '//www.youtube.com/embed/$1?wmode=transparent', {
            templateRegex: /.*(?:v\=|be\/|embed\/)([\w\-]+)&?.*/, embedtag: {tag: 'iframe', width: '100%', height: '280', params: '&rel=0'}
        }),
        ...
        new $.fn.oembed.OEmbedProvider("discoverychannel", "video", ["snagplayer\\.video\\.dp\\.discovery\\.com/.+"], "//snagplayer.video.dp.discovery.com/$1/snag-it-player.htm?auto=no", {templateRegex: [/.*snagplayer\.video\.dp\.discovery\/(\w+).*/], embedtag: {tag: 'iframe', width: 480, height: 360 }}),
        ...
        new $.fn.oembed.OEmbedProvider("viddy", "video", ["viddy\\.com/.+"], "//www.viddy.com/embed/video/$1", {templateRegex: [/.*viddy\.com\/embed\/video\/(\.*)/, /.*viddy\.com\/video\/(\.*)/], embedtag: {tag: 'iframe', width: 480, height: 360 }, nocache: 1}),
        new $.fn.oembed.OEmbedProvider("worldstarhiphop", "video", ["worldstarhiphop\\.com\/embed/.+"], "//www.worldstarhiphop.com/embed/$1", {templateRegex: /.*worldstarhiphop\.com\/embed\/(\w+).*/, embedtag: {tag: 'iframe', width: 480, height: 360 }, nocache: 1}),
        new $.fn.oembed.OEmbedProvider("zapiks", "video", ["zapiks\\.fr\/.+"], "//www.zapiks.fr/index.php?action=playerIframe&media_id=$1&autoStart=fals", {templateRegex: /.*zapiks\.fr\/index.php\?[\w\=\&]*media_id=(\w+).*/, embedtag: {tag: 'iframe', width: 480, height: 360 }, nocache: 1}),
        ...
        new $.fn.oembed.OEmbedProvider("Spotify", "rich", ["open.spotify.com/(track|album|user)/"], "//embed.spotify.com/oembed/"),
        ...
        new $.fn.oembed.OEmbedProvider("Soundcloud", "rich", ["soundcloud.com/.+", "snd.sc/.+"], "//soundcloud.com/oembed", { safeget: 'true', format: 'json' }),
        ...
        new $.fn.oembed.OEmbedProvider("flickr", "photo", ["flickr\\.com/photos/.+"], "//flickr.com/services/oembed", {safeget: 'true', callbackparameter: 'jsoncallback'}),
        ...
        // Commented because API was changed and seems that credentials are required for this API
        //new $.fn.oembed.OEmbedProvider("dribbble", "photo", ["dribbble.com/shots/.+"], "//api.dribbble.com/v1/shots/$1?callback=?",
        //    {
        ...
        new $.fn.oembed.OEmbedProvider("circuitlab", "photo", ["circuitlab.com/circuit/.+"], "//www.circuitlab.com/circuit/$1/screenshot/540x405/",
        ...
        new $.fn.oembed.OEmbedProvider("achewood", "photo", ["achewood\\.com\\/index.php\\?date=.+"], "//www.achewood.com/comic.php?date=$1", {templateRegex: /.*achewood\.com\/index.php\?date=(\w+).*/, embedtag: {tag: 'iframe', width: 480, height: 360 }, nocache: 1}),
        ...
        //Rich
        new $.fn.oembed.OEmbedProvider("twitter", "rich", ["twitter.com/.+"], "//api.twitter.com/1/statuses/oembed.json"),
        ...
        new $.fn.oembed.OEmbedProvider("documentcloud", "rich", ["documentcloud.org/documents/.+"], "//www.documentcloud.org/api/oembed.json"),
        ...
        new $.fn.oembed.OEmbedProvider("linkedin", "rich", ["linkedin.com/pub/.+"], "//www.linkedin.com/cws/member/public_profile?public_profile_url=$1&format=inline&isFramed=true",
        ...
        new $.fn.oembed.OEmbedProvider("github", "rich", ["gist.github.com/.+"], "//github.com/api/oembed"),
        new $.fn.oembed.OEmbedProvider("github", "rich", ["github.com/[-.\\w@]+/[-.\\w@]+"], "//api.github.com/repos/$1/$2?callback=?"
            , {templateRegex: /.*\/([^\/]+)\/([^\/]+).*/,
                templateData: function (data) {
        ...
        /*
         ...
         new $.fn.oembed.OEmbedProvider("facebook", "rich", ["facebook.com/(people/[^\\/]+/\\d+|[^\\/]+$)"], "//graph.facebook.com/$2$3/?callback=?"
        ...
        // Commented because there should be additional check for X-Frame-Options on building of embed code from this resource
        ...
        new $.fn.oembed.OEmbedProvider("slideshare", "rich", ["slideshare\.net"], "//www.slideshare.net/api/oembed/2", {format: 'json'}),
        ...
        new $.fn.oembed.OEmbedProvider("googleviews", "rich", ["(.*maps\\.google\\.com\\/maps\\?).+(output=svembed).+(cbp=(.*)).*"], "//maps.google.com/maps?layer=c&panoid=$3&ie=UTF8&source=embed&output=svembed&cbp=$5", {templateRegex: /(.*maps\.google\.com\/maps\?).+(panoid=(\w+)&).*(cbp=(.*)).*/, embedtag: {tag: 'iframe', width: 480, height: 360}, nocache: 1 }),
        ...
        new $.fn.oembed.OEmbedProvider("imajize", "rich", ["embed\\.imajize\\.com/.+"], "//embed.imajize.com/$1", {templateRegex: /.*embed\.imajize\.com\/(.*)/, embedtag: {tag: 'iframe', width: 480, height: 360 }, nocache: 1}),
        ...
        new $.fn.oembed.OEmbedProvider("ponga", "rich", ["ponga\\.com/.+"], "//www.ponga.com/embedded?id=$1", {templateRegex: [/.*ponga\.com\/embedded\?id=(\w+).*/, /.*ponga\.com\/(\w+).*/], embedtag: {tag: 'iframe', width: 480, height: 360 }, nocache: 1}),
        ...
        new $.fn.oembed.OEmbedProvider("opengraph", "rich", [".*"], null,
            {
                yql: {
                    ...
                    datareturn: function (results) {
                        ...
                        if (!results['og:title'] && !results['title'] && !results['url'])
                            return false;
                        ...
                        if (results['og:image'])
                        {
                            var imagePreview = $('<div>').addClass('ccl-embed-ogp-image-preview').css('background', 'no-repeat center/contain url(' + setHttp(results['og:image']) + ')');
                            container.append(imagePreview);
                        }

                        if (results['og:title']) {
                            var title = $('<div>').addClass('ccl-embed-ogp-header').html(results['og:title']);
                            container.append(title);
                        }

                        var descriptionText = results['og:description'] || results['description'];
                        if (descriptionText) {
                            var description = $('<div>').addClass('ccl-embed-ogp-description').html(descriptionText);
                            container.append(description);
                        }

                        if (results['og:title'] || descriptionText) {
                            var hostName = results['url'] ? getHostname(results['url']) : '';
                            var siteNameText = results['og:site_name'] || hostName;
                            if (siteNameText) {
                                var siteName = $('<div>').addClass('ccl-embed-ogp-site-name').html(siteNameText);
                                container.append(siteName);
                            }
                        }

                        if (container.html() === '' && container.text() === '' && results['url']) {
                            code.removeClass('ccl-embed-ogp-container');
                            var link = $('<div>').addClass('ccl-embed-ogp-dots').html(results['url']);
                            container.append(link);
                            isPlainLink = true;
                        }

                        return { code: code, isPlainLink: isPlainLink };
                    }
                }
            }
        )
